...
class PickAndPlaceClient(Node):
    ...
    # ==================== 1. 机械臂末端 6D 姿态控制 ====================
    def move_arm_to_pose(self, target_x, target_y, target_z, qx, qy, qz, qw):
        ...
        if not goal_handle.accepted:
            self.get_logger().error('❌ 机械臂运动目标被拒绝！')
            return False
...
def main(args=None):
    ...
    #qx, qy, qz, qw = 0.890122096, 0.455722125, 0.0, 0.0
    ...
    if not client.move_arm_to_pose(hover_x, hover_y, hover_z, qx, qy, qz, qw):
        client.get_logger().error('Step 1 失败，终止任务')
        return

    # ---------------- Step 2: 往下移动 8cm ----------------
    client.get_logger().info('⬇️ [Step 2/8] 正在往下移动 8cm 到抓取高度...')
    grasp_y = hover_y + DOWN_OFFSET_Y
    if not client.move_arm_to_pose(hover_x, grasp_y, hover_z, qx, qy, qz, qw):
        client.get_logger().error('Step 2 失败，终止任务')
        return

    # ---------------- Step 3: 控制夹爪关闭 (close) ----------------
    client.get_logger().info('✊ [Step 3/8] 正在关闭 6 轴夹爪 (close)...')
    client.control_gripper(GRIPPER_CLOSE)

    # ---------------- Step 4: 往上移动 8cm ----------------
    client.get_logger().info('⬆️ [Step 4/8] 正在往上抬升 8cm 回到悬空高度...')
    if not client.move_arm_to_pose(hover_x, hover_y, hover_z, qx, qy, qz, qw):
        client.get_logger().error('Step 4 失败，终止任务')
        return
    ...
    if not client.move_arm_to_pose(place_hover_x, place_hover_y, place_hover_z, qx, qy, qz, qw):
        client.get_logger().error('Step 5 失败，终止任务')
        return

    # ---------------- Step 6: 往下移动 8cm ----------------
    client.get_logger().info('⬇️ [Step 6/8] 正在往下移动 8cm 到放置高度...')
    #place_y = place_hover_y + DOWN_OFFSET_Y
    place_y = place_hover_y + 0.09
    if not client.move_arm_to_pose(place_hover_x, place_y, place_hover_z, qx, qy, qz, qw):
        client.get_logger().error('Step 6 失败，终止任务')
        return

    # ---------------- Step 7: 打开夹爪 (open) ----------------
    client.get_logger().info('✋ [Step 7/8] 正在打开 6 轴夹爪 (open)...')
    #time.sleep(0.3)
    client.control_gripper(GRIPPER_OPEN, wait_sec=0.5)

    # ---------------- Step 8: 往上移动 8cm 回到准备位置 ----------------
    client.get_logger().info('⬆️ [Step 8/8] 正在往上抬升 8cm 回到准备位置...')
    if not client.move_arm_to_pose(place_hover_x, place_hover_y, place_hover_z, qx, qy, qz, qw):
        client.get_logger().error('Step 8 失败，终止任务')
        return
